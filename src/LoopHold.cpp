  ...
  const CashoutBothDetails& cb) const
{
    ...
    completeList[cb.pLong][2] > completeList[cb.pShort][1])
  {
    ...
    return def.Set1(trick[0]);
      ...
      {
        xt0 = cb.numTopsLongHigh;
        xr0 = HR(cb.pLong, xt0-1);
      }
      xt1 = cb.lenShort - xt0;
      xr1 = HR(cb.pShort, cb.numTopsShortHigh);

      if (cb.lenOppLowest <= cb.lenShort)
        xr2 = SDS_VOID;
      else
        xr2 = HR(cb.pLong, cb.numTopsLongHigh);
      if (xr2 > xr1)
        xr2 = SDS_VOID;
/*
if (xt0 != l || xt1 != 3-l || cb.lenLong-cb.lenShort != 1)
{
  Holding::Print();
  cout << "t " << xt0 << "-" << xt1 << "-" << cb.lenLong-cb.lenShort << "\n";
  cout << "  " << l << "-" << 3-l << "-1\n";
  assert(false);
}
if (xr0 != r1 || xr1 != lowestRank || xr2 != SDS_VOID)
{
  Holding::Print();
  cout << "r " << xr0 << "-" << xr1 << "-" << xr2 << "\n";
  cout << "  " << r1 << "-" << lowestRank << "-" << SDS_VOID << "\n";
  assert(false);
}
*/
  ...
  {
    if (completeList[cb.pShort][cb.lenShort-1] <
        completeList[cb.pLong][cb.numTopsLongHigh] &&
        cb.numTopsLow > cb.lenOppLowest &&
        (cb.lenOppLowest <= cb.lenShort ||
        completeList[cb.pLong][cb.numTopsLongHigh+1] > cb.minOpp))
    {
      unsigned l = cb.numTopsLongHigh;
      if (cb.lenOppLowest > cb.lenShort)
        l++;
    ...
    cb.lenOppHighest == 2 && cb.lenOppLowest == 4)
  {
    if ((cb.numTopsLow == cb.lenOppMax &&
        ...
        (cb.numTopsLow >= cb.lenOppMax + 1 &&
        cb.numTopsShortHigh == 1 &&
        cb.numTopsShortLow == 3 &&
        cb.numTopsLongLow < 3))
    {
      if (pickFlag) holdCtr[0xa84]++;
      unsigned r0 = HR(cb.pLong, 0);
      // The way it comes out.
      unsigned t0 = (cb.pLong == QT_PARD ? 2 : 1);
      unsigned t1 = cb.lenShort - t0;
      unsigned r1 = HR(cb.pShort, 1);
      lowestRank = r1;
      trick[0].Set(QT_BOTH, cb.pLong, r0, t0);
      trick[1].Set(QT_BOTH, cb.pShort, r1, t1);
    ...
    cb.lenOppHighest == 1 && cb.lenOppLowest == 3)
  {
          ...
          completeList[cb.pShort][3])
    {
      ...
      if (cb.pShort == QT_ACE)
      {
        ...
        r1 = HR(QT_ACE, cb.numTopsShortLow);
      }
      else
      {
        if (cb.numTopsLongLow == 1)
...
bool LoopHold::SolveComplex48(DefList& def, unsigned& rank) const
{
      ...
      (length[QT_RHO] == 2 || length[QT_RHO] == 3))
  {
    if (htop.T == QT_PARD && htop.N == QT_PARD)
    {
      ...
      {
        // AKQx / 8xxx / T9 / Jx.
        // PROBLEM:  Should be rank 9, not rank T.
        if (pickFlag) holdCtr[0x1486]++;
        rank = SDS_VOID - 5;
        unsigned l = (length[QT_LHO] == 4 ? length[QT_ACE] : 4);
        trick[0].Set(QT_BOTH, QT_ACE, SDS_VOID-3, 3);
        trick[1].Set(QT_PARD, QT_PARD, SDS_VOID-5, 2);
        trick[2].Set(QT_ACE, QT_ACE, SDS_VOID, l-2);
        return def.Set12(trick[0], trick[1], trick[2]);
      }
    }
