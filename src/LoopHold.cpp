...
if (pickFlag) holdCtr[0xa99]++;
      ...
      completeList[cb.pLong][cb.lenOppLowest] > cb.maxPard)
  {
        ...
        cb.numTopsLongLow >= cb.lenOppLowest)
    {
      ...
      l = cb.lenOppLowest;
      unsigned r = Holding::ListToRank(completeList[QT_ACE][l]);
      lowestRank = Holding::ListToRank(cb.maxPard);
...
bool LoopHold::SolveComplex44(DefList& def, unsigned& rank) const
{
      ...
      completeList[QT_PARD][2] > completeList[QT_ACE][2])
  {
    ...
    else if (completeList[QT_PARD][2] > completeList[QT_ACE][2])
    {
      ...
      length[QT_RHO] == 2 && length[QT_LHO] == 2 &&
      htop.T == QT_PARD)
  {
    // AKxx+ / xx / JTx / Qx.
    ...
    return def.Set113(trick);
      ...
      ((length[QT_LHO] == 1 &&
       ...
       (length[QT_LHO] <= 3 && htop.T == QT_PARD &&
        completeList[QT_ACE][2] > completeList[QT_LHO][0])))
  {
    // AK8x(x) / xxx / JTx / Qx.
    // AK8x(x) / T / J9x / Qx.
  ...
  else if (length[QT_ACE] >= 5 && length[QT_PARD] == 3 &&
      length[QT_RHO] == 2 && length[QT_LHO] <= 3 &&
      htop.T == QT_ACE && 
      (htop.N == QT_PARD || 
  ...
  else if (distHex == 0x4432 && htop.T == QT_PARD && htop.N == QT_LHO)
  {
    if (htop.E == QT_ACE)
    {
      if (completeList[QT_PARD][2] > Max(completeList[QT_ACE][3],
        completeList[QT_LHO][1]))
      {
      ...
      else if (completeList[QT_ACE][3] > Max(completeList[QT_PARD][2],
          completeList[QT_LHO][1]))
      {
        // AK86 / 9654 / JT3 / Q7.
    ...
    else if (completeList[QT_PARD][2] > Max(completeList[QT_ACE][2],
        completeList[QT_LHO][1]))
    {
      // AK8x / 9xxx / JT7 / Qx.
...
bool LoopHold::SolveComplex48(DefList& def, unsigned& rank) const
{
      ...
      length[QT_RHO] == 3)
  {
    ...
    else if (htop.T == QT_PARD && htop.N == QT_PARD &&
          completeList[QT_ACE][3] > completeList[QT_PARD][3])
    {
      if (pickFlag) holdCtr[0x1482]++;
      rank = Holding::ListToRank(completeList[QT_ACE][3]);
      trick[0].Set(QT_ACE, QT_ACE, rank, length[QT_ACE]);
      trick[1].Set(QT_PARD, QT_BOTH, SDS_VOID-6, length[QT_ACE]);
