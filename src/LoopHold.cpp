        ...
        if (htop.N == pp)
        {
              ...
              completeList[pa][2] > completeList[pr][1])
          {
            ...
            if (pa == QT_ACE)
            {
              ...
              trick[1].Set(QT_BOTH, pa, SDS_VOID-1, 1);
...
bool LoopHold::SolveComplex15(DefList& def, unsigned& rank) const
{
  ...
  else if (length[QT_RHO] == 2)
  {
    ...
    else if (length[QT_ACE] < length[QT_PARD])
    {
      ...
      {
        // if (completeList[QT_LHO][1] > completeList[QT_PARD][1] &&
  ...
  else if (length[QT_RHO] == 3)
  {
    if (htop.T == QT_ACE)
    {
      if (htop.N == QT_PARD)
      {
        // PROBLEM (rank, the nine matters).
        if (pickFlag) holdCtr[0x1230]++;
        return false;
      }
      else if (htop.N == QT_ACE && htop.E == QT_PARD && length[QT_ACE] > 4)
      {
        // PROBLEM (rank, the eight matters).
        if (pickFlag) holdCtr[0x1231]++;
        return false;
      }
      else if (htop.N == QT_ACE && htop.E == QT_ACE &&
        completeList[QT_PARD][0] > completeList[QT_ACE][length[QT_ACE]-1])
      {
        // PROBLEM (rank, the seven matters).
        if (pickFlag) holdCtr[0x1232]++;
        return false;
      }
      else if (length[QT_PARD] == 3)
      {
        if (pickFlag) holdCtr[0x1233]++;
        rank = SDS_TEN;
        trick[0].Set(QT_ACE, QT_ACE, SDS_ACE, 1);
        trick[1].Set(QT_PARD, QT_ACE, SDS_QUEEN, 2);
...
bool LoopHold::SolveComplex18(DefList& def, unsigned& rank) const
{
      ...
      length[pl] == 4 && length[pr] <= 1)
  {
    ...
    {
      if (completeList[pl][1] > completeList[pa][3])
      {
        // The way it comes out.
        // AK9xx / J7xx / Q8x / T.
        ...
        else if (completeList[pp][2] < completeList[pa][2])
        {
          ...
          else if (length[QT_ACE] == 4)
          {
            ...
            if (completeList[pa][2] > completeList[pr][1])
            {
              ...
              rank = HR(pa, 2);
              trick[0].Set(QT_BOTH, pa, rank, 4);
              trick[1].Set(QT_BOTH, pa, r1, 1);
              trick[2].Set(QT_BOTH, pp, r2, 2);
              trick[3].Set(pa, pa, SDS_VOID, 1);
              return def.Set13(trick);
            }
            else
